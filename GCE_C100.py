...
parser.add_argument('--warmup_reg', type=int, default=22, help='Number of sweeps over the dataset to train')
    ...
    for i, (images1, images2,images3,labels,true_labels,indexes) in enumerate(train_loader):
        ...
        if args.base:
            if epoch<=args.warmup_base:
                ...
                loss = base_loss(F.softmax(output1, dim=1), labels)
        else:
            if epoch<=args.warmup_base:
                loss = criterion_ce(output1,labels)
                ...
                loss = args.alpha * base_loss(F.softmax(output1, dim=1), labels) + self_criterion(features2,features3) + reg_criterion(output1,features1)
